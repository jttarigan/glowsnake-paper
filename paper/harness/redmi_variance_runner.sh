...
OUT="$(cd "$(dirname "$0")/.." && pwd)/data/device/variance/redmi"
mkdir -p "$OUT"
LOG="$OUT/runner.log"
...
# the app may have been uninstalled since the original campaign
if ! $ADB shell pm list packages | grep -q $PKG; then
  say "app missing — installing release APK"
  if $ADB install -r "$(dirname "$0")/../../android/app/build/outputs/apk/release/app-release.apk" >> "$LOG" 2>&1; then
    say "install ok"
  else
    say "INSTALL FAILED (MIUI may need 'Install via USB' re-enabled)"; exit 1
  fi
fi
